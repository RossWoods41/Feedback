...
class SceneEvents_1 extends SceneScript
{
	...
	override public function init()
	{
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				g.setFont(getFont(33));
				g.strokeColor = Utils.getColorRGB(0,0,0);
				Script.setDrawingLayerToActorLayer(getActor(33));
				g.drawString("" + "Health:", 5, 460);
				g.drawString("" + "Score:", 510, 460);
				g.drawString("" + (Engine.engine.getGameAttribute("Score") : Float), 605, 460);
			}
		});
